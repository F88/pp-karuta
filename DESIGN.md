...
- **Shared theme**: Use shadcn/ui + Tailwind CSS variables (defined in `src/index.css`) for all pages.
- **Intro page theme**: The `/intro` route is intentionally isolated.
    - While the global `ThemeProvider` wraps the entire application (in `__root.tsx`), the Intro page enforces its own Matrix-style theme using specific CSS overrides (`!important`) defined in `src/components/intro/IntroPage.css`.

### Responsive Design

UIは以下の3つの画面サイズを意識したレスポンシブデザインを採用:

- **Smartphone** (base, ~639px): モバイル端末向け、コンパクトな表示
- **Tablet** (sm, 640px~1023px): タブレット端末向け、中サイズの表示
- **PC** (lg, 1024px~): デスクトップ向け、大きな表示

実装ではTailwind CSSのブレークポイント(`sm:`, `lg:`)を使用し、padding、テキストサイズ、アイコンサイズなどを段階的に調整する。
...
- `PromidasRepositoryManager` (Singleton) を使用してリポジトリのライフサイクルとトークン検証を管理
...
- `Yomite` コンポーネントおよび `useYomibito` フックにより制御される
- 画面上部に表示され、タイプライターエフェクトやMarqueeで提示される
...
- 入力方式により操作方法が異なる:
    - **Keyboard**: 各プレイヤーに割り当てられたキーで札を選択
    - **Touch**: 各プレイヤー専用のTatamiエリアをタップ
...
- PC環境向け、ショートカットキーで操作
- 共通のTatamiエリアを使用
- プレイヤー数とTatamiサイズに応じてキーレイアウトが適応される
    - **基本**: 数字キー、QWERTY行、ASDFG行、ZXCVB行を使用
...
2. 統合セレクター画面 (/)
   - PlayMode選択 (Keyboard/Touch)
   - Players選択 (1-4人、名前設定)
   - TatamiSize選択 (4/8/12/16枚など)
   - DeckRecipe選択 (データソース)
...
- PP Karutaの紹介テキスト
- 「ゲームを始める」ボタン
- 独立したテーマ（CSSによる強制オーバーライド）

### 2. 統合セレクター画面

#### レイアウト

縦方向にセクションが並ぶリスト形式レイアウトを採用。
`src/components/selector/integrated-selector-presentation.tsx` で実装。
...
3. **TatamiSize選択**
    - Tatamiに一度に表示する札の枚数を選択 (4, 8, 12, 16枚)
    - Keyboardモードのプレイヤー数に応じて選択可能なサイズが制限される
...
- **Replayボタン**:
    - 同じゲーム設定（Players/DeckRecipe/StackRecipe/TatamiSize）で即座にゲーム再開始
